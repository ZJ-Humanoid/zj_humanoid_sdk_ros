...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_robot_basic_info', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/robot/basic_info...")
    rospy.wait_for_service('/zj_humanoid/robot/basic_info')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/robot/basic_info', BasicInfo)
        
        # Create request
        req = dict_to_request(data, BasicInfoRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/robot/basic_info...")
