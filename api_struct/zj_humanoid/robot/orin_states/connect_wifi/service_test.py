...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_robot_orin_states_connect_wifi', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/robot/orin_states/connect_wifi...")
    rospy.wait_for_service('/zj_humanoid/robot/orin_states/connect_wifi')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/robot/orin_states/connect_wifi', ConnectWifi)
        
        # Create request
        req = dict_to_request(data, ConnectWifiRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/robot/orin_states/connect_wifi...")
