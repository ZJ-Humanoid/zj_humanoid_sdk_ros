...
import yaml
import sys
from robot.msg import RobotState
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('topic_test_zj_humanoid_robot_robot_state', anonymous=True)
    
    # Load data from YAML
    data = load_yaml_data(yaml_file)
    
    # Create publisher
    pub = rospy.Publisher('/zj_humanoid/robot/robot_state', RobotState, queue_size=10)
    
    # Create message
    msg = dict_to_msg(data, RobotState)
    
    # Publish at 10Hz
    rate = rospy.Rate(10)
    
    rospy.loginfo(f"Publishing to /zj_humanoid/robot/robot_state at 10Hz...")
    rospy.loginfo(f"Message type: robot/RobotState")
