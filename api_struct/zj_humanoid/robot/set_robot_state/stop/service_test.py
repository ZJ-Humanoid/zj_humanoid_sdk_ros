...
Service: /zj_humanoid/robot/set_robot_state/stop
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_robot_set_robot_state_stop', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/robot/set_robot_state/stop...")
    rospy.wait_for_service('/zj_humanoid/robot/set_robot_state/stop')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/robot/set_robot_state/stop', Trigger)
        
        # Create request
        req = dict_to_request(data, TriggerRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/robot/set_robot_state/stop...")
