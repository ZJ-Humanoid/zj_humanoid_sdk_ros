...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('topic_test_zj_humanoid_navigation_odom_info', anonymous=True)
    
    # Load data from YAML
    data = load_yaml_data(yaml_file)
    
    # Create publisher
    pub = rospy.Publisher('/zj_humanoid/navigation/odom_info', Odometry, queue_size=10)
    
    # Create message
    msg = dict_to_msg(data, Odometry)
    
    # Publish at 10Hz
    rate = rospy.Rate(10)
    
    rospy.loginfo(f"Publishing to /zj_humanoid/navigation/odom_info at 10Hz...")
    rospy.loginfo(f"Message type: nav_msgs/Odometry")
