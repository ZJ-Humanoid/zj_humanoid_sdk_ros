...
Service: /zj_humanoid/hand/joint_switch/right
Type: hand/HandJoint
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_hand_joint_switch_right', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/hand/joint_switch/right...")
    rospy.wait_for_service('/zj_humanoid/hand/joint_switch/right')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/hand/joint_switch/right', HandJoint)
        
        # Create request
        req = dict_to_request(data, HandJointRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/hand/joint_switch/right...")
