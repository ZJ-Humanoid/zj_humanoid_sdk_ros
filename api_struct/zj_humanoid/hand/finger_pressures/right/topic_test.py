...
Topic: /zj_humanoid/hand/finger_pressures/right
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('topic_test_zj_humanoid_hand_finger_pressures_right', anonymous=True)
    
    # Load data from YAML
    data = load_yaml_data(yaml_file)
    
    # Create publisher
    pub = rospy.Publisher('/zj_humanoid/hand/finger_pressures/right', PressureSensor, queue_size=10)
    
    # Create message
    msg = dict_to_msg(data, PressureSensor)
    
    # Publish at 10Hz
    rate = rospy.Rate(10)
    
    rospy.loginfo(f"Publishing to /zj_humanoid/hand/finger_pressures/right at 10Hz...")
    rospy.loginfo(f"Message type: hand/PressureSensor")
