...
Topic: /zj_humanoid/sensor/head_imu
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('topic_test_zj_humanoid_sensor_head_imu', anonymous=True)
    
    # Load data from YAML
    data = load_yaml_data(yaml_file)
    
    # Create publisher
    pub = rospy.Publisher('/zj_humanoid/sensor/head_imu', Imu, queue_size=10)
    
    # Create message
    msg = dict_to_msg(data, Imu)
    
    # Publish at 10Hz
    rate = rospy.Rate(10)
    
    rospy.loginfo(f"Publishing to /zj_humanoid/sensor/head_imu at 10Hz...")
    rospy.loginfo(f"Message type: sensor_msgs/Imu")
