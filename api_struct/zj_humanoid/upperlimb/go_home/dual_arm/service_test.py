...
Service: /zj_humanoid/upperlimb/go_home/dual_arm
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_upperlimb_go_home_dual_arm', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/upperlimb/go_home/dual_arm...")
    rospy.wait_for_service('/zj_humanoid/upperlimb/go_home/dual_arm')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/upperlimb/go_home/dual_arm', Trigger)
        
        # Create request
        req = dict_to_request(data, TriggerRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/upperlimb/go_home/dual_arm...")
