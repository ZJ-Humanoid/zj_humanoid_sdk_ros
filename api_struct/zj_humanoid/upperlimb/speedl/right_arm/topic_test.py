...
Topic: /zj_humanoid/upperlimb/speedl/right_arm
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('topic_test_zj_humanoid_upperlimb_speedl_right_arm', anonymous=True)
    
    # Load data from YAML
    data = load_yaml_data(yaml_file)
    
    # Create publisher
    pub = rospy.Publisher('/zj_humanoid/upperlimb/speedl/right_arm', SpeedL, queue_size=10)
    
    # Create message
    msg = dict_to_msg(data, SpeedL)
    
    # Publish at 10Hz
    rate = rospy.Rate(10)
    
    rospy.loginfo(f"Publishing to /zj_humanoid/upperlimb/speedl/right_arm at 10Hz...")
    rospy.loginfo(f"Message type: upperlimb/SpeedL")
