...
Topic: /zj_humanoid/upperlimb/servoj/whole_body
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('topic_test_zj_humanoid_upperlimb_servoj_whole_body', anonymous=True)
    
    # Load data from YAML
    data = load_yaml_data(yaml_file)
    
    # Create publisher
    pub = rospy.Publisher('/zj_humanoid/upperlimb/servoj/whole_body', Joints, queue_size=10)
    
    # Create message
    msg = dict_to_msg(data, Joints)
    
    # Publish at 10Hz
    rate = rospy.Rate(10)
    
    rospy.loginfo(f"Publishing to /zj_humanoid/upperlimb/servoj/whole_body at 10Hz...")
    rospy.loginfo(f"Message type: upperlimb/Joints")
