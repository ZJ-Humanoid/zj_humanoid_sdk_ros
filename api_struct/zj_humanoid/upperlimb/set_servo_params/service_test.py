...
import yaml
import sys
from upperlimb.srv import SetServoParams, SetServoParamsRequest
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_upperlimb_set_servo_params', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    service_name = '/zj_humanoid/upperlimb/set_servo_params'
    rospy.loginfo(f"Waiting for service {service_name}...")
    rospy.wait_for_service(service_name)
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy(service_name, SetServoParams)
        
        # Create request
        req = dict_to_request(data, SetServoParamsRequest)
