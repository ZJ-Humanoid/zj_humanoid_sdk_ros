...
Topic: /zj_humanoid/upperlimb/tcp_pose/right_arm
Type: upperlimb/Pose
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('topic_test_zj_humanoid_upperlimb_tcp_pose_right_arm', anonymous=True)
    
    # Load data from YAML
    data = load_yaml_data(yaml_file)
    
    # Create publisher
    pub = rospy.Publisher('/zj_humanoid/upperlimb/tcp_pose/right_arm', Pose, queue_size=10)
    
    # Create message
    msg = dict_to_msg(data, Pose)
    
    # Publish at 10Hz
    rate = rospy.Rate(10)
    
    rospy.loginfo(f"Publishing to /zj_humanoid/upperlimb/tcp_pose/right_arm at 10Hz...")
    rospy.loginfo(f"Message type: upperlimb/Pose")
