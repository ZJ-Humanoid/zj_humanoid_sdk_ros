...
Topic: /zj_humanoid/upperlimb/uplimb_state
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('topic_test_zj_humanoid_upperlimb_uplimb_state', anonymous=True)
    
    # Load data from YAML
    data = load_yaml_data(yaml_file)
    
    # Create publisher
    pub = rospy.Publisher('/zj_humanoid/upperlimb/uplimb_state', UplimbState, queue_size=10)
    
    # Create message
    msg = dict_to_msg(data, UplimbState)
    
    # Publish at 10Hz
    rate = rospy.Rate(10)
    
    rospy.loginfo(f"Publishing to /zj_humanoid/upperlimb/uplimb_state at 10Hz...")
    rospy.loginfo(f"Message type: upperlimb/UplimbState")
