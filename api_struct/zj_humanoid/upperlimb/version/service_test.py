...
Service: /zj_humanoid/upperlimb/version
Type: upperlimb/GetVersion
...
import yaml
import sys
from upperlimb.srv import GetVersion, GetVersionRequest
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_upperlimb_version', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    service_name = '/zj_humanoid/upperlimb/version'
    rospy.loginfo(f"Waiting for service {service_name}...")
    rospy.wait_for_service(service_name)
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy(service_name, GetVersion)
        
        # Create request
        req = dict_to_request(data, GetVersionRequest)
        ...
        rospy.loginfo(f"Version: {response.version}")
