...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_upperlimb_movej_neck', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/upperlimb/movej/neck...")
    rospy.wait_for_service('/zj_humanoid/upperlimb/movej/neck')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/upperlimb/movej/neck', MoveJ)
        
        # Create request
        req = dict_to_request(data, MoveJRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/upperlimb/movej/neck...")
