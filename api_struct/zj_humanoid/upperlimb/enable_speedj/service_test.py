...
Type: upperlimb/SetBool
Description: 使能speedj模式
...
import yaml
import sys
from upperlimb.srv import SetBool, SetBoolRequest
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_upperlimb_enable_speedj', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    service_name = '/zj_humanoid/upperlimb/enable_speedj'
    rospy.loginfo(f"Waiting for service {service_name}...")
    rospy.wait_for_service(service_name)
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy(service_name, SetBool)
