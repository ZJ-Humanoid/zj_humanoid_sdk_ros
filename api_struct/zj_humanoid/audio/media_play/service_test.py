...
Service: /zj_humanoid/audio/media_play
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_audio_media_play', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/audio/media_play...")
    rospy.wait_for_service('/zj_humanoid/audio/media_play')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/audio/media_play', MediaPlay)
        
        # Create request
        req = dict_to_request(data, MediaPlayRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/audio/media_play...")
