...
Service: /zj_humanoid/audio/LLM_chat
Type: audio/LLM_chat
Description: LLM对话服务
...
import yaml
import sys
from audio.srv import LLM_chat, LLM_chatRequest
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_audio_LLM_chat', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/audio/LLM_chat...")
    rospy.wait_for_service('/zj_humanoid/audio/LLM_chat')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/audio/LLM_chat', LLM_chat)
        
        # Create request
        req = dict_to_request(data, LLM_chatRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/audio/LLM_chat...")
