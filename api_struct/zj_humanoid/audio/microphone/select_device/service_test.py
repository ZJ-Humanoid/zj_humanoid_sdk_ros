...
Service: /zj_humanoid/audio/microphone/select_device
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_audio_microphone_select_device', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/audio/microphone/select_device...")
    rospy.wait_for_service('/zj_humanoid/audio/microphone/select_device')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/audio/microphone/select_device', SetDevice)
        
        # Create request
        req = dict_to_request(data, SetDeviceRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/audio/microphone/select_device...")
