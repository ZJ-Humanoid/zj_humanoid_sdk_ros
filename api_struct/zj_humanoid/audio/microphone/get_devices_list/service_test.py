...
Service: /zj_humanoid/audio/microphone/get_devices_list
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_audio_microphone_get_devices_list', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/audio/microphone/get_devices_list...")
    rospy.wait_for_service('/zj_humanoid/audio/microphone/get_devices_list')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/audio/microphone/get_devices_list', GetDeviceList)
        
        # Create request
        req = dict_to_request(data, GetDeviceListRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/audio/microphone/get_devices_list...")
