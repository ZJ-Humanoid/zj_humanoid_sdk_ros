...
| **Description** | 相机内外参标定 |
| **Note** | 自动相机内外参标定，外参标定时机器人会执行一段轨迹，拍摄不同角度的照片，从而计算外参。目前只支持U1型号机器人。|
...
| **Description** | 执行抓取服务 |
| **Note** | 输出物品名称执行抓取服务。目前只支持U1型号机器人。|
...
| **Description** | 视觉抓取示教 |
| **Note** | 视觉示教抓取，让机器人知道该从什么方位抓取物品。目前只支持U1型号机器人。|
...
| **Note** | 节空间轨迹规划，输出关节轨迹，示教模式下记录各个关节数据，据此生成完整的执行轨迹。目前只支持U1型号机器人。|
...
| **Description** | 获取目标位姿 |
| **Note** | 输入图像获取指定物品的6D位姿。目前只支持特定物品。|
...
| **Description** | 末端轨迹规划 |
| **Note** | 末端空间轨迹规划，示教模式下记录各个末端执行器数据，据此生成完整的执行轨迹。目前只支持U1型号机器人。|
...
| **Description** | 场景更新 |
| **Note** | 机器人场景更新,基于二维码，需要场景中有二维码，机器人抓取物品前的环境感知。目前只支持U1型号机器人。|
