...
string no            # 订单编号
int32 id             # 动作的序列id
time start_time      # 抓取动作序列的开始时间 
time end_time        # 抓取动作序列的结束时间
bool is_success      # 抓取动作序列是否成功
string msg           # 如果失败，希望能添加失败原因
```
#### `SingleItemInfo`
...
string no           # 订单编号
int32 item_id       # 商品ID
int32 index         # 一次订单中执行抓取任务的索引
string status       # 当前状态，取值范围[recog，grasp，nav，deliver]，分别为商品识别，抓取动作，导航， 交付
string msg          # 状态信息
bool result         # 状态结果
```
...
Header header
  ...
  string frame_id
string no           # 订单编号
int32 item_kinds    # 商品种类
retail/SingleItemInfo[] item_info # 商品信息
---
bool success        # 是否成功
string status       # 状态码[SUCCESS、RUNNING、INTERNAL_ERROR]
```
...
string metric_id          # 如"CTRL_0101_0001"
string module_id          # 如"01-01"
string data_type          # float/int/bool/string/float_array/int_array/bool_array/string_array/json
float64 float_value
int64 int_value
bool bool_value
string string_value
string json_value         # 记录类信息/无需引擎进一步处理的信息专用，避免滥用
float64[] float_values
int64[] int_values
bool[] bool_values
string[] string_values
string unit               # 单位，如"°"、"A"、"rpm"
float32 precision         # 精度，如0.1表示保留1位小数
float64 min_range         # 取值最小值
float64 max_range         # 取值最大值
uint8 category            # Metric自身分类：0=attribute（只希望传输数据）, 1=info, 2=warn, 3=error, 4=fatal
string state_machine      # 【可选】仅状态机类Metric填写（即attribute != 0时）
string state_level        # 【可选】仅状态机类Metric填写（即attribute != 0时）
string description        # Metric描述
...
```bash
float64[] joint         # 由于会存在多个关节的同时运动，所以该数据长度不能为7    单位:[rad]
```
