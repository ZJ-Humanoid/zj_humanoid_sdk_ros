...
Service: /zj_humanoid/audio/speaker/set_volume
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('service_test_zj_humanoid_audio_speaker_set_volume', anonymous=True)
    
    # Load request data from YAML
    data = load_yaml_data(yaml_file)
    
    # Wait for service
    rospy.loginfo(f"Waiting for service /zj_humanoid/audio/speaker/set_volume...")
    rospy.wait_for_service('/zj_humanoid/audio/speaker/set_volume')
    
    try:
        # Create service proxy
        service_proxy = rospy.ServiceProxy('/zj_humanoid/audio/speaker/set_volume', SetVolume)
        
        # Create request
        req = dict_to_request(data, SetVolumeRequest)
        
        rospy.loginfo(f"Calling service /zj_humanoid/audio/speaker/set_volume...")
