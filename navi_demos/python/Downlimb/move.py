...
class AutoController:
    ...
    def run(self):
        ...
        if self.direction == 'left':
            cmd_web_vel.linear.y = 0.3  # 使用与joy.yaml中相同的速度比例
        elif self.direction == 'right':
            cmd_web_vel.linear.y = -0.3  # 负值表示右移
        else:
        ...
        while abs(traveled_distance) < self.distance and not rospy.is_shutdown():
            ...
            traveled_distance = cmd_web_vel.linear.y * (rospy.get_time() - start_time)
            rate.sleep()
...
if __name__ == '__main__':
    try:
        ...
        if len(args) < 2 or len(args) > 3:
            ...
            print("<direction> can be 'left' or 'right'")
